...
from homr.onnxruntime_utils import rapidocr_params

# Globals
_reader: RapidOCR | None = None
_reader_uses_cuda: bool | None = None
...
def _initialize_reader(use_gpu_inference: bool = False) -> None:
    """
    Thread-safe initialization of the OCR reader.
    """
    global _reader, _reader_uses_cuda  # noqa: PLW0603
    params = rapidocr_params(use_gpu_inference)
    use_cuda = bool(params["EngineConfig.onnxruntime.use_cuda"])

    if _reader is None or _reader_uses_cuda != use_cuda:
        with _reader_lock:
            if _reader is None or _reader_uses_cuda != use_cuda:  # double-checked locking
                _reader = RapidOCR(params=params)
                _reader_uses_cuda = use_cuda
...
def _detect_title_task(debug: Debug, top_staff: Staff, use_gpu_inference: bool) -> str:
    _initialize_reader(use_gpu_inference)
...
def download_ocr_weights(use_gpu_inference: bool = False) -> None:
    """
...
def detect_title(debug: Debug, top_staff: Staff, use_gpu_inference: bool) -> Future[str]:
    """
    Runs the title detection in a separate thread and returns a Future.
    """
    return _executor.submit(_detect_title_task, debug, top_staff, use_gpu_inference)
